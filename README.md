...
## Microsoft Teams 채널 게시판

매주 이슈가 발행되면 `teams.py`가 다음을 수행합니다(설정된 것만).

1. **주간 Word 보고서** `docs/reports/<주차>.docx` 생성 — 인기 1~3위 + 분야별 전체 논문(모든 항목)
2. **채널 파일 폴더에 업로드** — OneDrive로 동기화한 채널 폴더(`config.json` `teams.sync_dir`)의
   `AI 논문 다이제스트/` 아래에 복사 → OneDrive 앱이 채널 '파일' 탭으로 올림
3. **채널 게시물(게시판)** — Teams 워크플로 웹훅(`.env` `TEAMS_WEBHOOK_URL`)으로 카드 게시
   (인기 1~3위 요약 + 분야별 목록 + [웹에서 보기] [주간 보고서] [채널 폴더] 버튼)

단계별 성공 여부를 `data/issues.json`에 기록하므로, 실패한 단계만 다음 실행 때 다시 시도합니다.
처음 연결하면 지난 주차 보고서도 순서대로 올라가 게시판에 이력이 채워집니다.

연결 상태 점검: `venv/bin/python teams_setup.py`

## 실패 대응 (자가 복구)

- 스케줄은 **매일 09:00** 실행. 이번 주 이슈가 이미 발행됐으면 바로 끝나고, 실패했으면 다음 날 다시 시도합니다.
- arXiv(분야 분류용)가 막히면 제목·초록 키워드로 분류해 계속 진행합니다. HF 날짜별 수집이라 과거 주차도
  `venv/bin/python main.py --week 2026-W37 --no-notify` 로 소급 생성할 수 있습니다.
- 다른 주차에 이미 실린 논문은 제외해 주차 간 중복이 없습니다.
